...
--local luadir = luadir
--local loadbitmap = loadbitmap
--local NineBitmapDrawable = NineBitmapDrawable
--local ArrayListAdapter = ArrayListAdapter
...
local function checkValue(var, default)
  return tonumber(var) or checkNumber(var) or default or var
end
...
local function layout(t,root,group)
  root=root or _G
  local view,style
  if t.style then
  ...
  if not t[1] then
    error(string.format("layout error: Fist value Must be a Class, checked import package.\n\tat %s",dump2(t)),0)
  end
  ...
  local params=ViewGroup_LayoutParams(checkValue(t.layout_width, -2), checkValue(t.layout_height, -2)) --设置layout属性
  if group then
    params = luajava.new(group.LayoutParams, params)
  end
  ...
  for k,v in pairs(t) do
    if tonumber(k) and type(v)=="table" then --创建子view
      view:addView(layout(v,root,t[1]))
    elseif k=="id" then --创建view的全局变量
      ...
      if not e then
        local _,i=s:find(":%d+:")
        s=s:sub(i or 1,-1)
        error(string.format("layout error %s \n\tat %s\n\tat key=%s value=%s\n\tat %s",s,view:toString(),k,v,dump2(t)),0)
      end
    end
  end
